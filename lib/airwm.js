...
var mapRequestHandler = function(ev){
	global.X.GetWindowAttributes(ev.wid, function(err, attributes) {
		...
		getWindowProperties(global.X, ev.wid).then(obj => {
			logger.info(JSON.stringify(obj));
			if (_.contains(obj['_NET_WM_WINDOW_TYPE'], '_NET_WM_WINDOW_TYPE_DESKTOP')) {
				logger.debug("window of a DESKTOP type")
				workspaces.getCurrentWorkspace().setDesktopWindow(ev.wid);
			}
			else if (_.contains(obj['_NET_WM_WINDOW_TYPE'], '_NET_WM_WINDOW_TYPE_DOCK')) {
				logger.debug("window of a DOCK type")
				workspaces.getCurrentWorkspace().addDockWindow(ev.wid, obj);
			}
			else {
