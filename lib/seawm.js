...
let statePrev = {};
let widgetsToShowPrev = [];
let widgetsToHidePrev = [];
let windowInfos = {};
function handleStateChange() {
	...
	const widgetsToShow = [];
	const widgetsToHide = [];
	_.forEach(state.widgets, (w, id) => {
		if (w.xid) {
			const info = {};
			const desktopId = getWidgetDesktopId(state, w);
			if (desktopId >= 0) {
				const desktop = state.widgets[desktopId.toString()];
				const screenId = desktop.screenId;
				if (screenId >= 0 && !_.isEmpty(w.rc)) {
					const desktopId = state.screens[screenId.toString()].desktopCurrentId;
					info.visible = true;
					info.rc = w.rc;
					info.desktopNum = state.desktopIds.indexOf(desktopId);
				}
				else {
					info.visible = false;
				}
	...
	_.forEach(state.screens, (screen, screenId) => {
		const desktopId = screen.desktopCurrentId;
		const desktop = state.wigets[desktopId.toString()];
		_
	});
}
