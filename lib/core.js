...
export function initialize(desktops, screens) {
	...
	for (let i = 0; i < screens.length; i++) {
		...
		_.merge(desktops[i], {
			screenId: i,
			rc: [0, 0, screens[i].width, screens[i].height]
		});
	}
...
export function updateLayout(state) {
	...
	state.get('screens').forEach((screen, screenId) => {
		const desktopId = screen.get('desktopCurrentId');
		state = state.setIn(['widgets', desktopId.toString(), 'rc'], List.of(0, 0, screen.get('width'), screen.get('height')));
	});
	...
	state.get('screens').forEach((screen, screenId) => {
		...
		if (true || desktop.get('layout', 'tile-right') === 'tile-right') {
			const childIds = desktop.get('childIds');
			...
			childIds.forEach((childId, i) => {
				...
				state1 = state1.setIn(['widgets', childId.toString(), 'rc'], List.of(
					x2, y, w2, h
				));
			});
		}
