...
function updateX11(state) {
	const focusWidgetId = state.get('focusWidgetId', -1);
	const focusWidgetKey = focusWidgetId.toString();
	state.get('widgets').forEach((w, key) => {
		const xid = w.get('xid');
		if (xid >= 0) {
			const hasFocus = (key === focusWidgetKey);
			const info = {
				xid: xid,
	...
	if (focusWidgetId >= 0) {
		const xid = state.getIn(['widgets', focusWidgetKey, 'xid']);
		state = state.setIn(['x11', 'wmSettings', 'SetInputFocus'], List(xid));
	}
	else {
		const screenCurrentId = state.get('screenCurrentId');
		const xid = state.getIn(['screens', screenCurrentId.toString(), 'xid']);
		state = state.setIn(['x11', 'wmSettings', 'SetInputFocus'], List(xid));
	}
