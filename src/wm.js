...
const getWindowProperties = require("../lib/getWindowProperties.js");
const StateWrapper = require("./StateWrapper.js");
const makeStore = require("./store.js");
const x11consts = require("./x11consts.js");
...
const programs = config.startup_applications;
		...
		global.X.InternAtom(false, "WM_S0", function(err, composite_atom) {
			...
			global.X.GetSelectionOwner(composite_atom, function(err, xid) {
				...
				if (xid > 0) {
					...
					global.X.ChangeWindowAttributes(xid, {
...
function clientCreator(err, display, xidRoot) {
	...
	try {
		...
		const action1 = {
			...
			screens: _.map(display.screen, screen => {
				return {
					xid: screen.root,
		...
		_.forEach(display.screen, (screen, i) => {
			global.X.AllocColor(screen.default_colormap, 0x5e00, 0x9d00, 0xc800, function(
				err,
				color
			) {
				store.dispatch({
					...
					screen: i,
					colors: { focus: color.pixel },
				});
			});
			global.X.AllocColor(screen.default_colormap, 0xdc00, 0xf000, 0xf700, function(
				err,
				color
			) {
				store.dispatch({
					type: "setX11ScreenColors",
					screen: i,
					colors: { normal: color.pixel },
				});
			});
			global.X.AllocColor(screen.default_colormap, 0x0c00, 0x2c00, 0x5200, function(
				err,
				color
			) {
				store.dispatch({
					type: "setX11ScreenColors",
					screen: i,
					colors: { alert: color.pixel },
				});
			});
			global.X.AllocColor(screen.default_colormap, 0xff00, 0x8000, 0x8000, function(
				err,
				color
			) {
				store.dispatch({
					type: "setX11ScreenColors",
					screen: i,
					colors: { floating: color.pixel },
				});
			});
			global.X.AllocColor(screen.default_colormap, 0xff00, 0xff00, 0x0000, function(
				err,
				color
			) {
				store.dispatch({
		...
		global.X.GetKeyboardMapping(min_keycode, max_keycode - min_keycode, function(
			err,
			key_list
		) {
		...
		global.X.ChangeWindowAttributes(
			...
			eventMask,
			changeWindowAttributeErrorHandler
		);
...
var grabKeyBindings = function(ks2kc, display) {
	logger.info(
		"Grabbing all the keybindings which are configured to have actions in the config.js file."
	);
	try {
		keybindings.forEach(function(keyConfiguration) {
			...
			for (var i in keyConfiguration.modifier) {
				keyConfiguration.mod =
					keyConfiguration.mod |
					conversion.translateModifiers(keyConfiguration.modifier[i]);
			}
...
function enqueueEvent(ev) {
	...
	if (
		ev.name === "MotionNotify" &&
		eventQueue.length > 0 &&
		_.last(eventQueue).name === "MotionNotify"
	) {
	...
	if (!eventHandlerRunning) handleEvent();
}
...
function handleEvent() {
	...
	try {
		let id = undefined;
		if (ev.wid) id = findWidgetIdForXid(ev.wid);
		...
		if (ev.name !== "MotionNotify" || eventNamePrev !== "MotionNotify") {
			...
			if (id === 6) console.log(_.omit(ev, "rawData"));
		}
...
function handleMotionNotify(ev) {
	if (dragStart) {
		...
		if (dragStart.keycode === 1) {
			...
			store.dispatch({
				type: "setWindowRequestedProperties",
				window: dragStart.id,
				props: { pos },
			});
		}
...
function handleClientMessage(ev) {
	return new Promise((resolve, reject) => {
		global.X.GetAtomName(ev.type, function(err, name) {
			...
			logger.info(
				"handleClientMessage: " + name + " " + JSON.stringify(_.omit(ev, "rawData"))
			);
			switch (name) {
...
function handleConfigureRequest(ev, id) {
	...
	if (id >= 0) {
		...
		const ConfigureWindow = state.getIn([
			"x11",
			"windowSettings",
			id.toString(),
			"ConfigureWindow",
		...
		if (window.flagFloating) {
			...
			if (ev.x > 0 || ev.y > 0) props.pos = [ev.x - borderWidth, ev.y - borderWidth];
	...
	if (id >= 0) X.ResizeWindow(ev.wid, width, height);
	return Promise.resolve();
}
				...
				if (isVisible || isIconified || hasDesktop) {
					...
					logger.info(
						"window isn't visible: " +
							JSON.stringify({ isVisible, isIconified, hasDesktop })
					);
				}
			});
		}
...
function handleStateChange() {
	try {
		...
		if (ignoreStateChange || eventHandlerRunning) return;

		const state = store.getState();
		if (state === statePrev) return;
		...
		windowIdStack.forEach(id => {
			...
			if (ChangeWindowAttributes !== settings0.get("ChangeWindowAttributes")) {
				...
				global.X.ChangeWindowAttributes.apply(
					global.X,
					ChangeWindowAttributes.toJS(),
					err => {
						if (err) {
							console.log(1);
							console.log({ err });
			...
			if (visible !== settings0.get("visible")) {
				if (visible) global.X.MapWindow(xid);
				else global.X.UnmapWindow(xid);
			}
		...
		if (
			SetInputFocus &&
			SetInputFocus !== statePrev.getIn(["x11", "wmSettings", "SetInputFocus"])
		) {
		...
		if (state !== store.getState()) handleStateChange();
	} catch (e) {
...
function findWindowIdForXid(xid) {
	...
	store
		...
		.forEach((w, key) => {
			if (w.get("xid") === xid) {
				if (w.get("type") === "window") id = parseInt(key);
				return false;
			}
		});
	return id;
